...
def make_planet(initial_X_coeffs, core_mass_coeffs, period_cdf, composition_coeffs):
    ...
    # composition ranges from -1 (pure ice) to 1 (pure iron)
    composition = composition_coeffs

    # envelope mass fraction from log-Gaussian
    (X_mean, X_stdev) = initial_X_coeffs
    X_initial = rand.lognormal(np.log(X_mean), X_stdev)

    # core mass from log-Gaussian
...
def CKS_synthetic_observation(N, distribution_parameters):
    ...
    if rank == 0:
        ...
        composition_params = distribution_parameters[0]
        period_params = [1.9,0.01,7.9]
        initial_X_params = distribution_parameters[1:3]
        core_mass_params = distribution_parameters[3:]

        KH_timescale = 100
        period_cdf = period_distribution_CDF(period_params[0],
                                             period_params[1],
                                             period_params[2])
        ...
        while transit_number < N:
            ...
            X_initial, M_core, period, M_star, composition = make_planet(initial_X_coeffs=initial_X_params,
                                                                         core_mass_coeffs=core_mass_params,
                                                                         period_cdf=period_cdf,
                                                                         composition_coeffs=composition_params)
