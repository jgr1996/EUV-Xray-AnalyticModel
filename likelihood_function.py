...
def run_single_population(N, counter):
    # get number of cores
    ...
    np.savetxt('./RESULTS/{0}/R_array_{1}.csv'.format(current_time_string, counter), R, delimiter=',')
    np.savetxt('./RESULTS/{0}/P_array_{1}.csv'.format(current_time_string, counter), P, delimiter=',')
